...
| `lockTimeout`       | `1800000` (30min) | Maximum time since a job was claimed (`lockedAt`) before it is considered stale |
...
Monque manages heartbeats automatically in the background. If a heartbeat update fails (e.g., due to database connectivity), Monque emits a `job:error` event.
...
```typescript
// Index for stale job detection (optimizes queries on lockedAt)
...
- [Change Streams](/monque/advanced/change-streams/) - Real-time notifications
- [Graceful Shutdown](/monque/getting-started/quick-start/#handle-graceful-shutdown) - Clean worker termination
