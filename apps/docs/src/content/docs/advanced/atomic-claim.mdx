...
**Why stale recovery runs first:** A crashed instance leaves jobs in `processing` with a stale heartbeat. Running stale recovery before the collision check resets those jobs to `pending`, preventing false positives from crash-recovery scenarios.

<Aside type="caution">
  Always use a unique `schedulerInstanceId` per running instance. Sharing an ID across active instances causes a `ConnectionError` on startup of the second instance.
</Aside>
