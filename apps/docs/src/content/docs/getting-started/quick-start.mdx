...
<Aside type="caution">
  **Idempotency Required:** Monque provides at-least-once delivery. Jobs may run more than once due to retries or [stale recovery](/monque/advanced/heartbeat/). Design your workers to be idempotent—safe to run multiple times with the same input. See [Idempotent Operations](/monque/core-concepts/jobs/#idempotent-operations) and the [Production Checklist](/monque/advanced/production-checklist/).
</Aside>
...
const monque = new Monque(db, {
  collectionName: 'jobs',
  pollInterval: 1000,
  safetyPollInterval: 30000,   // Safety poll interval when Change Streams active
  maxRetries: 10,
  ...
  maxPayloadSize: 100_000,   // Reject payloads over 100 KB
  statsCacheTtlMs: 5000,     // Cache getQueueStats() results for 5 s
});
```
