...
The `@monque/tsed` package provides a seamless integration between [Monque](https://github.com/debruyn/monque) and [Ts.ED](https://tsed.io/), allowing you to define workers and cron jobs using decorators and leverage dependency injection.
...
| [`@WorkerController()`](/monque/api-tsed/functions/workercontroller/) | Register a worker controller |
| [`@Worker()`](/monque/api-tsed/functions/worker/) | Register a job handler |
...
| [`@InjectMonque()`](/monque/api-tsed/functions/injectmonque/) | Property decorator to inject the service |
...
Create a class decorated with [`@WorkerController`](/monque/api-tsed/functions/workercontroller/). Related job handlers are grouped here, similar to how REST controllers group HTTP endpoints. Methods decorated with [`@Worker`](/monque/api-tsed/functions/worker/) will process jobs.
...
@WorkerController("email") // Optional namespace prefix: "email."
export class EmailWorkers {
  @Worker("send", { concurrency: 5 }) // Full job name: "email.send"
...
Inject [`MonqueService`](/monque/api-tsed/classes/monqueservice/) into any Ts.ED Service or Controller to dispatch jobs. You can use either standard `@Inject()` or the specialized [`@InjectMonque()`](/monque/api-tsed/functions/injectmonque/) property decorator.

```typescript
import { Service, Inject } from "@tsed/di";
import { MonqueService, InjectMonque } from "@monque/tsed";
...
Use `@tsed/platform-http/testing` and `PlatformTest` to test your workers. You can mock `MonqueService` or use a real MongoDB connection with `Testcontainers`.
