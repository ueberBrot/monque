...
```
nextRunAt = now + (2^failCount × baseRetryInterval)
```
...
<Aside type="tip">
  Use `maxBackoffDelay` to cap extremely long delays. With the default `maxRetries: 10`, the last actual retry occurs when `failCount = 9` (a 512s ≈ 8.5min delay). If you increase `maxRetries`, the delay for `failCount = 10` would be 1024s (≈ 17min).
</Aside>
