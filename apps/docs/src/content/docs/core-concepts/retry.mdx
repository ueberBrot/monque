...
|-------------------|---------|-------------|
| [`maxRetries`](/monque/api/interfaces/monqueoptions/#maxretries) | `10` | Maximum retry attempts |
...
<Aside type="tip">
  Monque caps exponential backoff by default to prevent unbounded delays (e.g. `failCount = 20` at a 1s base is > 11 days) and to avoid precision/overflow issues at very large `failCount` values. Override with `maxBackoffDelay` if you want a smaller (or larger) cap.
</Aside>
...
- [Change Streams](/monque/advanced/change-streams/) - Real-time monitoring
