...
<Aside type="caution">
  Cron expressions are evaluated in the server's local timezone unless the environment is specifically configured for UTC. For consistent behavior across deployments, it is recommended to set your server or container timezone to UTC.
</Aside>
