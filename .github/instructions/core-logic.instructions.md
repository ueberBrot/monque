...
```typescript
const MAX_BACKOFF_INTERVAL = 24 * 60 * 60 * 1_000; // 24 hours (configurable via maxBackoffDelay)
const delay = Math.min(Math.pow(2, job.failCount) * baseRetryInterval, MAX_BACKOFF_INTERVAL);
...
- On timeout: Jobs still in-progress MUST be left as `PROCESSING` for stale job recovery mechanisms to re-claim and reschedule.