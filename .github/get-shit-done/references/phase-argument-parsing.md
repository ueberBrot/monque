...
```bash
PHASE_INFO=$(node ".github/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ".github/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ".github/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
