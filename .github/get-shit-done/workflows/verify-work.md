...
```bash
INIT=$(node ".github/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
Commit the UAT file:
```bash
node ".github/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
