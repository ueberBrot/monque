...
monque.worker('validate-input', async (job) => {
  // Use job.failCount to skip retries for known unrecoverable errors
  if (job.failCount > 0) {
    return; // Returning instead of throwing "skips" the retry
  }

  if (!isValid(job.data)) {
    // Throwing MonqueError will trigger the retry policy
    throw new MonqueError('Invalid input');
  }
});
```

<Aside type="tip">
  Checking `job.failCount` inside your `monque.worker` handler is the recommended way to prevent retries for errors that won't be resolved by trying again, instead of throwing `MonqueError`.
</Aside>
